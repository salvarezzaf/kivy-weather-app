...
class CurrentCondition(BoxLayout):
    ...
    def parse_current_weather_data(self, request, data):
        ...
        if "celsius" in self.db_util.get_default_tem_unit():
            self.current_temp = str(int(data['current_observation']['temp_c'])) + self.degree_symbol
        else:
            self.current_temp = str(int(data['current_observation']['temp_f'])) + self.degree_symbol

    def parse_high_low_temperature(self, request, data):
        high_temp = str(data['forecast']['simpleforecast']['forecastday'][0]['high'][self.db_util.get_default_tem_unit()])
        low_temp = str(data['forecast']['simpleforecast']['forecastday'][0]['low'][self.db_util.get_default_tem_unit()])
